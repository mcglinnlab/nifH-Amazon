                 ...
                 distance.metric = 'euclidean', univariate=FALSE)
{
  ...
  ## Arguments:
  ## x: an object of class 'sim' or may be a site x sp matrix, a vector of values,
  ##   a dissimilarity object of class 'dist', if x is a vector of values then
  ##   missing samples should be coded as -999
  ...
  if (class(x) == "sim"){
    ...
    x = matrix(x)
  else if (!is.matrix(x) & class(x) != 'dist')
    stop('x must be either of class sim, a vector, a distance object, or a matrix')
  ...
  if (univariate) {
    ...
    if (!is.vector(x) & class(x) != 'dist') { ## i.e. x is a site x sp matrix and not simply a vector
